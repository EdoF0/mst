...
PreorderTree è generato secondo una visita anticipata (anche detta visita in preordine o pre-order tree trasversal) del MST ordinata secondo il peso dell'arco e, in caso di parità, in ordine lessicografico.
...
graph_arcs(G, As).
```
Vero se As è la lista di tutti gli archi di G.  
...
```
Crea un grafo G a partire da un file csv FileName con carattere separatore tabulazione, dove ogni riga è composta da vertice 1, vertice 2 e peso dell'arco dal vertice 1 al vertice 2.  
...
```
Se il terzo argomento è 'graph' crea un file csv FileName a partire da un grafo G; se il terzo argomento è 'edges' crea un file csv FileName da una lista di archi G.  
...
Fallisce se non esiste H, oppure se la chiave K non è un numero, oppure se K o V sono variabili.

#### heap_extract
```prolog
heap_extract(H, K, V).
```
Elimina dal minheap H il valore V con chiave K. I valori K e V possono essere definiti oppure lasciati entrambi variabili per una operazione di extract standard sulla testa del minheap.  
